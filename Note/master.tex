...
\item \begin{definition}[Certificate of Optimality I (Bound)]\label{optimality:i}
...
\item \begin{definition}[Certificate of Optimality II (Weak Duality)]\label{optimality:ii}
...
\item \begin{definition}[Certificate of Optimality III (CS Conditions)]\label{optimality:iii}
...
\begin{definition}[Certificate of Infeasibility (Farkas' Lemma)]\label{infeasibility}
...
\begin{definition}[Certificate of Unboundedness]\label{unboundedness}
...
\begin{definition}[Simplex: Phase I]\label{phase:i}
...
\begin{definition*}
  \textbf{Degeneracy}
\item \begin{definition}[Degenerate Iteration]\label{degeneracy:i}
  A degenerate iteration is an iteration where the BFS does not change after the iteration.
\end{definition}

\item \begin{definition}[Degenerate Solution]\label{degeneracy:ii}
...
\begin{eg}[Finding the Dual]\label{eg}
Find the dual of: \\
\begin{equation*}
  \begin{aligned}
    \min & \m{1 & 2 & 3}x \\
...
\begin{algorithm}
  ...
  \algrenewcommand\algorithmicrequire{\textbf{Input: $G = (V, E)$ and $c \in \mathbb R^m$}}
  \algrenewcommand\algorithmicensure{\textbf{Output: an s,t path with minimum cost}}
  ...
  \begin{algorithmic}[1]
    ...
    \While{$t \notin U$} 
      ...
      \State Find the edge $uv$ with minimum slack \\ \Comment{If there are multiple edges with the same slack, choose one arbitrarily}
      \State Set $y_U := \text{slack}(uv)$
      \State Set $U := U \cup \{v\}$, and $T := T \cup \{uv\}$ 
    \EndWhile \Comment{At the end of this loop, we have a shortest s,t path in $T$}
...
\begin{algorithm}
  \floatname{algorithm}{Algorithm}
  \algrenewcommand\algorithmicrequire{\textbf{Input: } An IP: }
...
\begin{proposition}[Properties of Gradients and Subgradients]\label{prop}
. \\
  \begin{itemize}
...
\begin{definition}[Supporting Halfspace]\label{supporting}
...
\begin{corollary}[NLP Relaxation]\label{nlp}
...
\item \begin{definition}[KKT Conditions for a General NLP]\label{kkt}
...
\item \begin{definition}[KKT Conditions for a General NLP II]\label{kkt}
...
\item \begin{definition}[KKT Conditions for an NLP without Equality Constraints]\label{kkt2}
  Given an NLP in the form of: \\ $\{\min f(x) : g_i(x) \leq 0, i = 1, \dots, m\}$, 
  The KKT conditions are split into 4 parts:
  \begin{enumerate}
    \item Stationarity: $0 \in \partial f(x) + \sum_{i=1}^m \lambda_i \partial g_i(x)$
    \item Primal Feasibility: $g_i(x) \leq 0$ for all $i$
    \item Dual Feasibility: $\lambda_i \geq 0, i = 1, \dots, m$.
    \item Complementary Slackness: $\lambda_i g_i(x) = 0, i = 1, \dots, m$
  \end{enumerate}
\end{definition}

\item \begin{definition}[KKT Conditions for an NLP without Equality Constraints II]\label{kkt2}
  Let $\bar x$ be a solution to the NLP. Also assume that the NLP has a Slater Point. Then the ``only" KKT condition is:
\begin{center}
  $-\nabla f(x) \in \text{cone}\left\{ \nabla g_i(x): i \in T \right\}$, 
\end{center}
where T is the index set of tight constraints at $\bar x$.
\end{definition}
\end{definition*}
...
\begin{theorem}[KKT Theorem]
  Let $x^*$ be a solution to an NLP. Then $x^*$ is optimal if and only if the KKT conditions hold.
\end{theorem}
